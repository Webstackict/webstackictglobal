...
export async function enrollStudent(prevState, formData) {
    ...
    const referral_code = formData.get("referral_code")?.toString().trim();

    console.log("Enrollment attempt received:", { full_name, email, phone, program, cohort_id });
    ...
    try {
        ...
        if (error) {
            console.error("Supabase enrollment error detail:", {
